...
pub trait Component: Send + Sync + 'static {
    ...
    /// Compile-time unique identifier for this component type.
    ///
    /// Generated by `#[derive(Component)]` from `fnv1a_hash(concat!(module_path!(), "::", type_name))`.
    /// Used for compile-time constraint validation in [`Bundle`](crate::bundle::Bundle) impls.
    const UID: u128 = 0;

    /// UIDs of components required by this component's constraints.
    ///
    /// When non-empty, any [`Bundle`](crate::bundle::Bundle) containing this component
    /// must also contain all components listed here, or compilation fails.
    const REQUIRE_UIDS: &'static [u128] = &[];

    /// UIDs of components forbidden by this component's constraints.
    ///
    /// When non-empty, any [`Bundle`](crate::bundle::Bundle) containing this component
    /// must NOT contain any component listed here, or compilation fails.
